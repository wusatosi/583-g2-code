...
def parse_arguments():
  ...
  parser.add_argument('--only', dest='only', action='store', default='.*', help='regex of benchmarks to include (default=".*")')
  parser.add_argument('--path', dtype=str, default='test/polybench-cpu', help='path to polybench-cpu')
...
def PolybenchRootDir() -> Path:
  ...
  #return Path("/home/jespark/taffo/TAFFO/test/polybench-cpu")
  return Path(__file__).absolute().parent.parent / 'test/polybench-cpu'


def BenchmarkList(rootdir=Path(__file__).absolute().parent.parent / 'test/polybench-cpu'):
  blpath = Path(rootdir).joinpath(Path('./utilities/benchmark_list'))
...
def calc_validate_dataframe(filepath, regex=".*"):
  """
  Run validation and return pandas dataframe
  """
  g_res = {}
  for bench in BenchmarkList(filepath):
    if not re.search(regex, bench):
      continue
  ...
  df = pd.DataFrame.from_dict(g_res)
  df = df.transpose()
  return df
